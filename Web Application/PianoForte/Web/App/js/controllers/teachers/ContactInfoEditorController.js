...
goog.provide('PianoForte.Controllers.Teachers.ContactInfoEditorController');

PianoForte.Controllers.Teachers.ContactInfoEditorController = function ($scope, $rootScope, TeacherService, Enum, EnumConverter, FormatManager, ValidationManager) {
	$scope['Enum'] = Enum;
	...
	var onSuccessDeleteTeacherContactInfo = function(data, status, headers, config) {
		...
		if (isSuccess === true) {
			...
			if ((_requestInsertContactList.length === 0) &&
				...
				$rootScope.$broadcast('UpdateTeacherContactInfo', _teacherId, _contacts);
